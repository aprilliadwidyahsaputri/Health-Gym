...
const EditBlogForm = ({ route }) => {
  ...
  const dataCategory = [
    ...
    { id: 3, name: "Membangun otot" },
    { id: 4, name: "Membentuk tubuh" },
    { id: 5, name: "Seluruh tubuh" },
  ];
  ...
  useEffect(() => {
    const subscriber = firestore()
      ...
      .onSnapshot(documentSnapshot => {
        ...
        if (blogData) {
          console.log('jenis latihan data: ', blogData);
          setBlogData({
          ...
          console.log(`jenis latihan dengan id ${blogId} tidak ada.`);
        }
      });
    setLoading(false);
    return () => subscriber();
  ...
  const handleUpdate = async () => {
    ...
    const reference = storage().ref(`discoverimages/${filename}`);
    try {
      ...
      await firestore().collection('discover').doc(blogId).update({
        title: blogData.title,
      ...
      setLoading(false);
      console.log('Jenis Latihan Diupdate!');
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <View style={[textInput.borderDashed, { minHeight: 50 }]}>
          <TextInput
            placeholder="Target latihan"
            value={blogData.target}
